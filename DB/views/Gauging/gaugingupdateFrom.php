...
<div><br></div>
  ...
  padding: 15px;
  margin: auto;">
  <h5 class="card-header" align = "center">เพิ่มสถานีจุดตรวจวัดระดับน้ำ</h5>
            <div class="card-body">
            <div class="mb-3">

<form method = "get"action="">
    <br>
    <label>ชื่อสถานีตรวจ<input type="text" name="gauging_name" class="form-control" value="<?php echo $gauging->gauging_name;?>"/></label><br>
    

    <br>
    <label>ชื่อแม่น้ำ<select name="river_id" class="form-control" required>
        <?php 
        echo "<option value = $gauging->river_name";
        foreach ($river as $rv) 
        {
            echo "<option value = $rv->river_id> $rv->river_name </option>";
        }?>
    </select></label><br>

    <br>
    <input type = "hidden" name = "controller" value="gauging"/>
    <input type = "hidden" name = "gaugingstation_id" value="<?php echo $gauging->gaugingstation_id?>"/>
    <div class="col-sm">
